...
class PrescriptionHelper {
  ...
  static Future<List<Map<String, dynamic>>> getPrescriptionByid(int id) async {
    final db = await SQLHelper.db();
    return db.query('prescription',
  ...
  static Future<List<Prescription>> preparePrescriptionObject(
    ...
    for (int i = 0; i < map.length; i++) {
      ...
      await MedicationHelper.getMedicationByPrescritionId(tempid)
        ...
        for (var med in value1) {
          ...
          await MedicineHelper.getMedicineById(ttt.medicine_id!).then((medic) {
